...
#define RAW_LOGGER(worker, ...) \
  worker->logger(__FILE__, __FUNCTION__, __LINE__, __VA_ARGS__)
#else
#define RAW_LOGGER(worker, ...)
#endif
...
inline string bin_format(const string &var) {
  ...
  return is_output_to_terminal ? "\033[1;31m" + converted + "\033[1;37m"
                               : "[" + converted + "]";
}
...
class Worker {
 private:
  ...
  void set_content(const string &prefix, const int call_id, const int obj_id,
                   const string &content);
  string get_args_repr(const int call_id, const int obj_id);
  void set_args_repr(const int call_id, const int obj_id,
  ...
  static bool calc_enable_py_logger() {
    ...
    return (enable_py_logger != nullptr &&
            std::string(enable_py_logger) == "True");
  }
  ...
  int call_worker_func(const int worker_id, const function_ids func_id,
                       const Message *args, const bool need_wait = true);
 ...
 public:
  Worker(const string &host, const string &port, const string &server_id,
         const string &studio_url, const string &pool_type,
         const string &redis_url, const int max_pool_size,
         const int max_expire_time, const unsigned int max_timeout_seconds,
         const unsigned int num_workers);
  ~Worker();

  template <typename... Args>
  void logger(const string &file_name, const string &func_name,
              const int line_num, Args... args) {
    if (_enable_logger) {
      ...
      std::cout << "port = " << _port << " worker_id = " << std::setw(3)
                << std::setfill(' ') << _worker_id
      ...
      std::cout << " - " << (is_output_to_terminal ? "\033[1;37m" : "") << msg
                << (is_output_to_terminal ? "\033[0m" : "") << std::endl;
    }
  }

  string call_create_agent(const string &agent_id,
  ...
  pair<bool, string> call_agent_func(const string &agent_id,
