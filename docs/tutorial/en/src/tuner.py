...
How to Implement
~~~~~~~~~~~~~~~~~~~
Here we will implement a simple math agent workflow that can be trained using the ``tuner`` module.
...
async def example_workflow_function(
    ...
    Args:
        ...
        model (OpenAIChatModel): The chat model used by the agent.
        auxiliary_models (Optional[Dict[str, OpenAIChatModel]]): Additional
            chat models for auxiliary purposes.

    Returns:
        dict: The output generated by the workflow.
    """
    agent = ReActAgent(
        name="react_agent",
